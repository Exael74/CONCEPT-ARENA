package com.conceptarena.game.infra.nlp;
 ...
 * comparison (case, accents, and whitespace insensitive). NOTE: despite the class name, this
 * does not call an external NLP service — it's the monolith's original normalized string-match
 * implementation, carried forward as-is. A REST call to an external semantic-similarity service
 * (e.g. sentence-transformers) could replace this without changing the AnswerValidationPort
 * contract; evaluate whether the project scope actually wants that before renaming/reimplementing.
 */
@Component
