...
class OutboxEventPublisherTest {
    ...
    private OutboxEvent pendingEvent() {
        return new OutboxEvent("evt-1", "user-1", "UserRegistered", "conceptarena.auth.events",
            "auth.user-registered", "{\"aggregateId\":\"user-1\"}", Instant.now(), "corr-1");
    }
    ...
    void publishesPendingEventThenMarksItPublished() {
        ...
        verify(rabbitTemplate).convertAndSend(eq("conceptarena.auth.events"), eq("auth.user-registered"),
            eq("{\"aggregateId\":\"user-1\"}"), any(MessagePostProcessor.class));
