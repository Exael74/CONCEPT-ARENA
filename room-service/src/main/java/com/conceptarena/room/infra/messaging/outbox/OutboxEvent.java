...
/**
 * The ONLY table in room-service's Postgres database — Room/Participant domain state lives
 * entirely in Redis (see ADR-003). This table exists purely so event publication survives a
 * crash between the Redis write and the RabbitMQ publish, same pattern as every other service's
 * outbox — see ADR-002.
 */
@Entity
